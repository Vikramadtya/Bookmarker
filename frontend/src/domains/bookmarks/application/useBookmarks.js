...
export function useBookmarks(folderId, q = "", fields = []) {
  return useQuery({
    ...
    // Data stays fresh for 2 minutes. Without staleTime, React Query marks
    // the cache stale immediately and re-fetches on every component re-mount.
    staleTime: 1000 * 60 * 2,
    queryFn: async () => {
...
export function useTags() {
  return useQuery({
    queryKey: ["tags"],
    staleTime: 1000 * 60 * 5, // Tags rarely change — cache for 5 minutes
    queryFn: async () =>
      makeApiRequest({
...
export function useCreateBookmark() {
  ...
  return useMutation({
    ...
    onError: () => {
      // Toast is already shown by the Axios interceptor — no double-toast.
      // Callers can still pass their own onError via mutate() options.
    },
  });
}
